            ...
            if read.cigartuples[0][0] == 4: # 4 is a soft clip
                if len(start_window) == 0:
                    ...
                    if len(start_window) > min_threshold and len(start_window) < max_threshold:
                        ...
                        s_s = s_s[::-1]
                        # print(s_s)
                        if len(s_s) > min_char_clipped:                                                 
                            junctions.append(['+', start_window[0][0], s_s, len(start_window)-1])
                ...
                if len(end_window) == 0:
                    ...
                    if len(end_window) > min_threshold and len(end_window) < max_threshold:
                        ...
                        # print(s_e)
                        if junctions[-1][1] == end_window[0][0]:
                            junctions.pop()
                        if len(s_e) > min_char_clipped:
                            junctions.append(['-', end_window[0][0], s_e, len(end_window)-1])
